          ...
          <FadeIn className="lg:col-span-7 xl:col-span-8">
            <div className="relative">
              <AnimatePresence mode="wait">
                ...
                >
                  <Link 
                    to={`/projecten/${featuredProjects[activeIndex].slug}`}
                    className="group block relative aspect-[4/3] md:aspect-[16/10] rounded-2xl md:rounded-3xl overflow-hidden"
                  >
                    {/* Image */}
                    <img 
                      src={featuredProjects[activeIndex].imageAfter} 
              ...
              {featuredProjects.map((project, index) => (
                <FadeIn key={project.slug} delay={index * 0.1}>
                  <button
                    ...
                    className={`group w-full text-left p-3 md:p-4 rounded-xl md:rounded-2xl border transition-all duration-300 ${
                      index === activeIndex 
                        ? 'bg-slate-900 border-slate-900' 
                        : 'bg-slate-50 border-slate-200 hover:border-brand-green/30 hover:bg-slate-100'
                    }`}
                  >
                    <div className="flex items-start gap-3 md:gap-4">
                      ...
                      <div className="relative w-16 h-16 md:w-20 md:h-20 rounded-lg md:rounded-xl overflow-hidden shrink-0">
                        ...
                        />
                        {index === activeIndex && (
                          <div className="absolute inset-0 bg-brand-green/20 border-2 border-brand-green rounded-lg md:rounded-xl" />
                        )}
                      </div>
                      ...
                      <div className="flex-1 min-w-0">
                        <span className={`text-[9px] md:text-[10px] font-bold uppercase tracking-wider ${
                          index === activeIndex ? 'text-brand-green' : 'text-slate-400'
                        }`}>
                          {project.category}
                        </span>
                        <h4 className={`text-sm md:text-base font-heading uppercase tracking-tight truncate mt-1 ${
                          index === activeIndex ? 'text-white' : 'text-slate-900'
                        }`}>
                          {project.location}
                        </h4>
                        <div className={`flex items-center gap-1 mt-1 text-xs ${
                          index === activeIndex ? 'text-white/60' : 'text-slate-500'
                        }`}>
                          <MapPin size={10} />
                      ...
                      <ArrowRight 
                        ...
                        className={`shrink-0 transition-all ${
                          index === activeIndex 
                            ? 'text-brand-green' 
                            : 'text-slate-300 group-hover:text-brand-green group-hover:translate-x-1'
                        }`} 
                      />
                    </div>
                  </button>
