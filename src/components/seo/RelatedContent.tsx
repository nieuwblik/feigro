        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {items.slice(0, 5).map((item, index) => (
            <Link
              key={index}
              to={item.href}
              className="group block p-6 bg-slate-50 border border-slate-200 rounded-2xl hover:border-brand-green/30 hover:bg-white hover:shadow-lg transition-all duration-300"
            >
