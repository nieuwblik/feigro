    ...
    <section id="diensten" className="py-20 md:py-28 lg:py-36 overflow-hidden border-t border-slate-100 relative bg-slate-100">
      <div className="container mx-auto px-4 md:px-6">
        <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-8 mb-16 md:mb-24">
          ...
          <div className="flex flex-wrap items-center gap-6 md:gap-14 lg:pb-1 shrink-0">
            {[
              { val: '24u', label: 'Noodservice' },
              { val: '20+ jr', label: 'Innovatie' }
            ].map((stat, i) => (
              <FadeIn key={i} delay={0.2 + (i * 0.1)} distance={10}>
                <div className="flex flex-col items-start">
                  <p className="text-slate-900 lg:text-brand-green text-3xl md:text-4xl font-heading mb-0.5 leading-none tracking-tighter">{stat.val}</p>
                  <p className="text-slate-400 text-[8px] md:text-[9px] uppercase tracking-[0.2em] font-bold">{stat.label}</p>
                </div>
              </FadeIn>
            ))}
          </div>
